...
1. CAJE00_투자자본상계 : 그대로 유지.
2. CAJE01_채권채무제거 : 전기 행을 삭제하고 당기전기에 당기를 전기로 변경. 
3. CAJE02_제품미실현이익제거 : FS_Element A항목 금액 합계를 이 시트에 들어 있는 FS_Element X 해당 항목 계정을 찾아 그 계정으로 쓰고 이익잉여금(CoA "AJE"시트에 입력되어 있는). 부호는 유지. (기존 데이터는 삭제하고 이것으로 대체) 당기전기에 당기를 전기로 변경. 
4. CAJE03_상각자산미실현이익제거 : 모든 행 데이터 유지. FS_Element X 해당 항목(감가상각비)의 - 를 이익잉여금(CoA "AJE"시트에 입력되어 있는)으로 변경. 당기전기에 당기를 전기로 변경. 
5. CAJE04_배당조정 : 기존 데이터를 삭제.
6. CAJE96_취득일차이조정 : FS_Element X 해당 항목 행의 - 를 이익잉여금(CoA "AJE"시트에 입력되어 있는)으로 변경. 당기전기에 당기를 전기로 변경.
7. CAJE97_법인세조정 : 전기와 당기 행을 모두 제거. 전기 당기가 아닌 것(취득일)은 유지. 업로드 된 파일 내용의 다른 조정에서 각 항목에 맞게 새로 이월조정을 생성. CAJE02_제품미실현이익제거 에서는 FS_Element A 해당 항목의 금액에 FS_Element A 해당 세율을 반영하여 - 를 각각 법인세비용과 이익잉여금으로 기재(부호 같게).  CAJE03_상각자산미실현이익제거 에서는 누적되어 있는 모든 항목을 반영. FS_Element의 R, X 항목(당기손익)을 찾아서 X인 경우 FS_Element A 해당 세율을 반영하여 - 를 이익잉여금(CoA "AJE"시트에 입력되어 있는)으로 +를 이연법인세부채 CAJE97_법인세조정에 입력(부호 다르게). CAJE96_취득일차이조정에서는 누적되어 있는 모든 항목을 반영. FS_Element의 R, X 항목(당기손익)을 찾아서 X인 경우 FS_Element R, X 항목 해당 세율을 반영하여 - 를 이익잉여금(CoA "AJE"시트에 입력되어 있는)으로 +를 이연법인세부채 CAJE97_법인세조정에 입력(부호 다르게).  
8. CAJE98_비지배지분조정 : 당기를 전기로 바꾸고 비지배지분순이익을 비지배지분(CE)의 계정코드와 계정명으로 변경. 
