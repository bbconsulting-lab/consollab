...
* CAJE00_투자자본상계 : 그대로 유지.
* CAJE01_채권채무제거 : 전기 행을 삭제하고 당기전기에 당기를 전기로 변경. 
* CAJE02_제품미실현이익제거 : FS_Element A항목 금액 합계를 이 시트에 들어 있는 FS_Element X 해당 항목 계정을 찾아 그 계정으로 쓰고 이익잉여금(CoA "AJE"시트에 입력되어 있는). 부호는 유지. (기존 데이터는 삭제하고 이것으로 대체) 당기전기에 당기를 전기로 변경. 
* CAJE03_상각자산미실현이익제거 : 모든 행 데이터 유지. FS_Element X 해당 항목(감가상각비)의 - 를 이익잉여금(CoA "AJE"시트에 입력되어 있는)으로 변경. 당기전기에 당기를 전기로 변경. 
* CAJE04_배당조정 : 기존 데이터를 삭제.
* CAJE96_취득일차이조정 : FS_Element X 해당 항목 행의 - 를 이익잉여금(CoA "AJE"시트에 입력되어 있는)으로 변경. 당기전기에 당기를 전기로 변경.
* CAJE97_법인세조정 : 전기와 당기 행을 모두 제거. 전기 당기가 아닌 것(취득일)은 유지. 업로드 된 파일 내용의 다른 조정에서 각 항목에 맞게 새로 이월조정을 생성. CAJE02_제품미실현이익제거 에서는 FS_Element A 해당 항목의 금액에 FS_Element A 해당 세율을 반영하여 - 를 각각 법인세비용과 이익잉여금으로 기재(부호 같게).  CAJE03_상각자산미실현이익제거 에서는 누적되어 있는 모든 항목을 반영. FS_Element의 R, X 항목(당기손익)을 찾아서 X인 경우 FS_Element A 해당 세율을 반영하여 - 를 이익잉여금(CoA "AJE"시트에 입력되어 있는)으로 +를 이연법인세부채 CAJE97_법인세조정에 입력(부호 다르게). CAJE96_취득일차이조정에서는 누적되어 있는 모든 항목을 반영. FS_Element의 R, X 항목(당기손익)을 찾아서 X인 경우 FS_Element R, X 항목 해당 세율을 반영하여 - 를 이익잉여금(CoA "AJE"시트에 입력되어 있는)으로 +를 이연법인세부채 CAJE97_법인세조정에 입력(부호 다르게).  
* CAJE98_비지배지분조정 : 당기를 전기로 바꾸고 비지배지분순이익을 비지배지분(CE)의 계정코드와 계정명으로 변경. 


## 4) 자본변동표 작성 Logic
* 조정분개 파일의 당기전기 열에 당기가 아닌 것들은 기초 조정으로 반영한다. 
* 당기 이외 조정분개의 자본항목(FS_Element E)에 대하여 L3_code로 맵핑한다.
* 자본변동표 2번째 행(계정코드)와 CoA L3_code 가 일치하도록 설계되어 있도록 엑셀 구현.
* 자본변동표 2번째 행(계정코드)와 비지배지분(FS_Element CE)에 따라서 4번째 행 (Beginning)에 조정분개가 반영되도록 한다.
* 모회사 및 자회사들의 기초 금액을 모두 합산하고 기초 누적 조정분개를 반영되도록 하여 첫번째 줄(4번째 행 -Beginning)에 쓴다.
* 모회사 및 자회사  Beginning~Ending 사이의 내용을 모두 Concat으로 붙이고 당기연결 조정분개 중에서 손익에 영향을 미치는 항목은 "CE11_NI" 의 CE 계정코드로 이익잉여금 열(가장 오른쪽 직전)에 위치하게 하고 비지배지분순이익은 비지배지분(가장 오른쪽) 열에 위치하도록 한다. 손익에 영향이 없는 자본항목 조정분개는 다음 줄에 "CE12_CAJE"로 나타나게 한다.
* 모든 열의 행이 비어 있는 경우(0) 행을 제거한다.
* Ending 행에 각 열의 합게를 계산하여 나타나게 한다.
* Ending 행 다음 행에 자본 계정코드(2번째 행) 해당 연결정산표 금액이 나타나게 한다.
